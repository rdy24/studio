...
interface VoyageContextType {
	...
	getVoyageNameById: (voyageId: string) => string | undefined;
...
export const VoyageProvider: React.FC<{ children: React.ReactNode }> = ({
	...
	React.useEffect(() => {
		try {
			...
			if (storedVoyages) {
				const parsedVoyages: Voyage[] = JSON.parse(storedVoyages).map(
					(voyage: any) => ({
						...voyage,
						startDate: voyage.startDate
							? new Date(voyage.startDate)
							: undefined,
						endDate: voyage.endDate
							? new Date(voyage.endDate)
							: undefined,
				...
				localStorage.setItem(
					LOCAL_STORAGE_KEY,
					JSON.stringify(initialVoyagesData)
				);
			}
		} catch (error) {
	...
	const updateVoyage = (updatedVoyage: Voyage) => {
		setVoyages((prevVoyages) =>
			prevVoyages.map((voy) =>
				voy.id === updatedVoyage.id ? updatedVoyage : voy
			)
		);
	};
	...
	const getVoyageNameById = (voyageId: string): string | undefined => {
		const voyage = voyages.find((voy) => voy.id === voyageId);
		return voyage?.name;
	};
