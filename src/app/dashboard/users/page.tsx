...
const UserForm = ({
	...
	const [roleId, setRoleId] = React.useState(
		user?.roleId ?? (availableRoles.length > 0 ? availableRoles[0].id : "")
	);
	...
	const handleSubmit = () => {
		if (!roleId) return; // Jangan submit jika roleId kosong
		const userData = {
			name,
			email,
			roleId,
	...
	return (
		<>
			...
			<DialogFooter className="pt-4">
				...
				<Button type="submit" onClick={handleSubmit} disabled={!roleId}>
					Save user
				</Button>
			</DialogFooter>
		</>
...
export default function UserManagementPage() {
	const { users, addUser, updateUser, deleteUser, initialUsersLoaded } =
		useUserContext();
