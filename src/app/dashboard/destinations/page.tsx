...
export default function DestinationManagementPage() {
	...
	const handleSaveDestination = async (destination: Destination) => {
		...
		try {
			if (editingDestination) {
				const result = await updateDestination(editingDestination.id, {
				...
				if (result) {
					toast({
						...
						description: `Destination '${result.name}' has been successfully updated.`,
					});
				...
				if (result) {
					toast({
						...
						description: `Destination '${result.name}' has been successfully added.`,
					});
					setIsFormOpen(false);
				}
			}
	...
	const totalPages =
		totalDestinations > 0
			? Math.ceil(totalDestinations / ITEMS_PER_PAGE)
			: 1;
		...
		<div className="space-y-6 w-full p-4 md:p-6 lg:p-8">
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<h1 className="text-3xl font-bold text-primary tracking-tight flex items-center gap-2">
					<MapPin className="h-7 w-7" /> Destination Management
				</h1>
				<Dialog
					open={isFormOpen}
					onOpenChange={(isOpen) => {
				...
				>
					...
					<DialogContent className="sm:max-w-lg">
						<DialogHeader>
							...
							<DialogDescription>
								{editingDestination
									? "Modify details of the existing destination."
									: "Define a new travel destination."}
							</DialogDescription>
						</DialogHeader>
						<DestinationForm
						...
						<TableBody>
							{paginatedDestinations.map((destination) => (
								...
								>
										...
										>
											<AvatarImage
												src={
													...
													`https://placehold.co/40x40.png?text=${destination.name.charAt(
														0
													)}`
												}
									...
									<TableCell className="text-sm text-muted-foreground max-w-xs truncate p-2 sm:p-4 hidden md:table-cell">
										{destination.description}
									</TableCell>
			...
			{totalDestinations > 0 && initialDestinationsLoaded && (
				<div className="flex flex-col sm:flex-row items-center justify-between space-y-2 sm:space-y-0 sm:space-x-2 py-4">
					<span className="text-sm text-muted-foreground">
						Page {totalDestinations > 0 ? currentPage : 0} of{" "}
						{totalPages}
					</span>
					<div className="flex items-center space-x-2">
						<Button
							...
							disabled={
								currentPage === 1 || totalDestinations === 0
							}
						>
